...
public class StateSetAssetAllocation {
	...
	Map<Integer, MDPCapitalMarketsState> stateSet;
	int size;
	ArrayList<MDPCapitalMarketsState> statesDomain;
	
	
	
	public StateSetAssetAllocation(){
		this.stateSet = new LinkedHashMap<>();
		this.statesDomain = new ArrayList<MDPCapitalMarketsState>();
		size = 0;
	}
	 ...
	 * @param t
	 */
	public void put(MDPCapitalMarketsState t){
		
		if(t instanceof MDPCapitalMarketsState){ // check if the IState is an instance of AssetAllocationMDPState. This should not be required for the most part
...
//				System.out.println("failed to insert " + t.toString());
	 ...
	 public Map<Integer, MDPCapitalMarketsState> StatesSet(){
	 ...
	 public ArrayList<MDPCapitalMarketsState> getStatesDomain(){
